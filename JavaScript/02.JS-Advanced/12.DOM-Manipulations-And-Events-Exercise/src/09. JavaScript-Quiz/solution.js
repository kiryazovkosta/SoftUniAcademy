function solve() {
  ...
  Array.from(document.getElementsByClassName('answer-text')).forEach(x => x.addEventListener('click', selectAnswer));
  let result = '';
  let rightAnswers = 0;
  ...
  function selectAnswer(ev) {
    ...
    if (answeredQuestions === 3) {
      ...
      result = rightAnswers === 3 ? `You are recognized as top JavaScript fan!` : `You have ${rightAnswers} right answers`;
