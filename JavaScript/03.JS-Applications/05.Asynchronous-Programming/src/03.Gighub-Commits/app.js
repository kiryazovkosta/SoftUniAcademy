...
async function loadCommits() {
    ...
    try {
        ...
        commitsUlElement.replaceChildren();
        const response = await fetch(`https://api.github.com/repos/${usernameElement.value}/${repoElement.value}/commits`);
        if (response.ok) {
            const data = await response.json();
            data.forEach(commit => {
                commitsUlElement.appendChild(createElement('li', `${commit.commit.author.name}: ${commit.commit.message}`));
            });
        } else {
            throw new Error(`${response.status} (${ httpStatus[response.status]})`);
        }
    } catch (error) {
        commitsUlElement.appendChild(createElement('li', error));
    } finally {
